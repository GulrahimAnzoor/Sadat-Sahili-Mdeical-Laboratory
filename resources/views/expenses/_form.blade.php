...
<div class="grid gap-4 sm:grid-cols-3">
    <div>
        ...
        <select name="category" class="lab-input">
            @foreach (['rent' => __('Rent'), 'electricity' => __('Electricity'), 'fuel' => __('Fuel'), 'salary' => __('Salary'), 'other' => __('Other')] as $value => $label)
                <option value="{{ $value }}" @selected(old('category', $expense->category ?? 'other') === $value)>{{ $label }}</option>
            @endforeach
        </select>
    </div>
    <div>
    ...
    <div>
        ...
        <input name="spent_on" type="date" required class="lab-input" value="{{ old('spent_on', isset($expense) ? $expense->spent_on->toDateString() : now()->toDateString()) }}">
    </div>
</div>
<div>
